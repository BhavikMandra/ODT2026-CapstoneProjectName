# esp32_main.py — Luxo firmware: BLE (+ optional UART) JSON → servos + NeoPixel
# ─────────────────────────────────────────────────────────────
# JSON matches luxo_v2/lamp_link.py (BleLampController):
#   {"command":"behavior","name":"..."}  {"command":"move","joint":"pan|tilt|lower|mid","angle":N}
#   {"command":"music_bounce"}  → runs behavior "wiggle"
#   {"command":"neo_set"|"neo_flash"|"neo_emotion"|"neo_state", ...}
# BLE → Nordic UART GATT write + newline. UART2 → GPIO16/17 @ 115200 + newline.
# ─────────────────────────────────────────────────────────────
...
# USB serial heartbeat: link + RX stats + lamp snapshot (ms between lines; 0 = off).
LAMP_DIAG_INTERVAL_MS = 2000

# Shorter = idle motions start sooner after boot (was 15000).
...
# ══════════════════════════════════════════════════════════════
#   STEPPER CLASS (safety cap — infinite loop here froze idle + BLE)
# ══════════════════════════════════════════════════════════════
...
class Stepper:
    ...
    def move_to(self, target, step_delay_us=None):
        ...
        if n >= guard and self.pos != target:
            print("⚠️  Stepper: stop at guard steps (target=%d pos=%d)" % (target, self.pos))
...
def servo_dispatcher():
    global _busy
    print("⚙️  Servo dispatcher started")
    while True:
        behavior_cmd = None
        move_cmds = {}
        with _cmd_lock:
        ...
        if behavior_cmd:
            ...
            print("⚙️  dispatcher: running behavior %r" % nm)
            run_behavior(nm)
        elif move_cmds:
            while True:
                with _servo_lock:
            ...
            if u.any():
                ...
                while b"\n" in buf:
                    ...
                    if not line:
                        continue
                    if len(line) > 600:
                        print("⚠️  UART line too long, skip")
                        continue
                    try:
...
# RX counters for USB heartbeat (gatt = raw chunks from central; json_ok = full lines parsed).
_diag_ble_gatt_chunks = 0
_diag_ble_json_ok = 0
_diag_ble_json_err = 0
# GATT server write path (why gatt_chunks might stay 0 while ble=linked).
_diag_ble_gatt_write_irq = 0
_diag_ble_gatt_wr_mismatch = 0
_diag_ble_gatt_wr_empty = 0
_last_lamp_diag_ms = 0
# Incoming BLE payloads (IRQ → main). Do not use _thread locks from BLE IRQ.
_ble_irq_chunks = []
_ble_line_acc = b""
...
# BLE IRQ event values (MicroPython docs). Some ESP32 builds expose no bluetooth.IRQ_* on the
# module, or a file named bluetooth.py on the filesystem shadows the C module — use literals in IRQ.
...
def ble_process_rx():
    ...
    if len(_ble_line_acc) > 1200:
        print("⚠️  BLE rx buffer overflow, clearing")
        _ble_line_acc = b""
        return
    while b"\n" in _ble_line_acc:
        ...
        if not line:
            continue
        if len(line) > 600:
            print("⚠️  BLE line too long, skip")
            continue
        try:
            cmd = json.loads(line.decode())
            _diag_ble_json_ok += 1
